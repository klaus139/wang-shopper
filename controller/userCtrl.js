...
const userCart = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const user = await User.findById(_id);
    const alreadyExistCartId = user?.cart; // Assuming the cart ID is stored in the `cart` field of the User model

    if (alreadyExistCartId) {
      await Cart.findByIdAndDelete(alreadyExistCartId);
    }
